...
You can do this using the JDK jar tool (or any other ZIP extraction tool). In the example below we are assuming you downloaded the zip file was named restcomm-slee-{project-version}-wildfly-24.0.1.Final.zip to the /restcomm directory.
+

[source,subs="attributes"]
----
[usr]$ cd /restcomm
[usr]$ jar -xvf restcomm-slee-{project-version}-wildfly-24.0.1.Final.zip
----

. Setting up JBOSS_HOME Environment Variable
+
You should now have a directory called restcomm-slee-{project-version}-wildfly-24.0.1.Final.
