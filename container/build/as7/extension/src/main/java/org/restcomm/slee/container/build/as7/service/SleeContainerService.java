...
public class SleeContainerService implements Service<SleeContainer> {
	...
	private MobicentsClusterFactory initClusterFactory(ClassLoader classLoader) {
		...
		try {
			...
			log.error("Configuration could not be decoded ", e2);
		}

		int cacheExecutorThreads = getPropertyInt("CacheExecutorConfiguration", "cacheExecutorThreads", 128);
		long cacheExecutorTerminateTimeout = getPropertyLong("CacheExecutorConfiguration", "cacheExecutorTerminateTimeout", 1000L);
		...
		return new MobicentsClusterFactory(this.cacheConfig, cacheConfigData, configuration, globalConfig,
